...
import com.megapelis.images.model.dto.request.UpdateImageRQ;
...
public class UpdateImageHandler extends ImageHandler implements RequestHandler<Request, Response> {
    ...
    public Object validatePayload(Request request) throws ImageException {
        UpdateImageRQ updateImageRQ = convertPayload(request, UpdateImageRQ.class);
        if (!ImageCommon.isValidString(updateImageRQ.getId(), updateImageRQ.getImage()))
            throw new ImageException(ImageStatusEnum.ERROR_FORMAT_REQUEST);
        return updateImageRQ;
    }
    ...
    public Response handleRequest(Request request, Context context) {
        ...
        System.out.println("Image to update: " + image.getId());
        Image imageDB = mapper.load(Image.class, image.getId());
        imageDB.setImage(image.getImage());
